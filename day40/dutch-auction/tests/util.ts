...
export async function confirmAndPrintTxDetails(
    ...
    try {
        ...
        if (transaction?.meta) {
            ...
            let detail ={
                signature,
                computeUnits: meta.computeUnitsConsumed ?? null, // 兼容未消耗CU的场景
                fee: meta.fee,
                logs: meta.logMessages ?? null
            };
            console.log("signature=", signature, "fee=",meta.fee);
            console.log(chalk.blueBright("transcation details:" ));
            for(let log of detail.logs){
                console.log(chalk.blue(log))
            }
        }else{
            console.log(chalk.red("not find transcation details"));
        }
...
export async function printAccount(conn,pubKey:PublicKey,prompt = ""){
    ...
    if(!accountInfo){
        console.log(prompt,`Account of Address ${pubKey.toBase58()}:`, null);
        return;
    }
    console.log(prompt,
...
export async function generateKeypairWithSol(conn:Connection,amount:number){
    const keypair = anchor.web3.Keypair.generate();
    await airdropSol(conn,keypair.publicKey,amount * 1e9);

    return keypair;
}

export async function printTokenAccount(conn:Connection, tokenAccountPubkey:PublicKey, prompt = ""){
    if(prompt) console.log(prompt);

    let tokenAccount = await getAccount(conn, tokenAccountPubkey);
